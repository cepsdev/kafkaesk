...
static bool ws_int_on_read_callback(int oc,
  ...
  ws_int.message_read(oc,buffer,len,ws,stream_id);
}
        ...
        int stream_id)
{
    ...
    for(;;){
        ...
        if (ready == -1) if(errno == EINTR) continue; else fatal("epoll_wait");

        if(ready <= 0) continue;
        //first check for a signaled ctrl pipe
        bool done = false;
        for(size_t cur_ev = 0;cur_ev != ready;++cur_ev){
        ...
        for(size_t cur_ev = 0;cur_ev != ready;++cur_ev){
                ...
                auto state = socket2state[sck].first;
                ...
                if (ios.write_pending() && evlist[cur_ev].events & EPOLLOUT){
                    if (ios.out_buffer.empty() || ios.out_buffer.size() <= ios.out_pos){
                        epoll_event ev;
                        ev.events = EPOLLOUT;
                        ev.data.fd = sck;
                        if(epoll_ctl(epfd,EPOLL_CTL_MOD,sck,&ev) == -1);
                    ...
                    if (ios.out_pos >= ios.out_buffer.size()){
                        ios.write_finished();
                    }
